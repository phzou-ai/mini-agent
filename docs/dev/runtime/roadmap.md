...
The staged path beyond the current correctness work is defined in
[runtime-evolution-path.md](runtime-evolution-path.md). That document provides
feasibility and activation criteria; this roadmap remains authoritative for
...
This table is the authoritative summary of completed work and the active
priority. Detailed milestone rationale and acceptance criteria remain below.
...
The following order applies after R3.2:
...
[local-process-transitions.md](local-process-transitions.md).
