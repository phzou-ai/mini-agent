...
| Approval and ordinary input | `tasks/resume` can consume only an approval continuation; `SendMessage` with `taskId` can consume only ordinary task input, without rerouting as a new top-level request. | `tests/test_main_agent_core.py`, `tests/test_api_a2a_adapter.py`. |
