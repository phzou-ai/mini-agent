...
from vermay.execution_context import ExecutionCancellation, ExecutionContext
from vermay.infra import ssh as ssh_module
from vermay.infra.ssh import SshClient
...
def clear_ssh_env(monkeypatch):
    for key in SSH_ENV_KEYS:
        monkeypatch.delenv("VERMAY_SSH_" + key, raising=False)
...
def test_ssh_client_loads_env_local_config(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / ".env.local").write_text(
        "\n".join(
            [
                "VERMAY_SSH_TARGET=user@example-host",
                "VERMAY_SSH_PORT=2222",
                "VERMAY_SSH_IDENTITY_FILE=~/.ssh/example",
                "VERMAY_SSH_KNOWN_HOSTS_FILE=~/.ssh/known_hosts",
            ]
        ),
        encoding="utf-8",
    )
...
def test_ssh_client_env_vars_override_env_files(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / ".env.local").write_text(
        "\n".join(
            [
                "VERMAY_SSH_TARGET=file-host",
                "VERMAY_SSH_PORT=22",
                "VERMAY_SSH_IDENTITY_FILE=~/.ssh/file",
                "VERMAY_SSH_KNOWN_HOSTS_FILE=~/.ssh/known_hosts",
            ]
        ),
        encoding="utf-8",
    )
    monkeypatch.setenv("VERMAY_SSH_TARGET", "env-host")
    monkeypatch.setenv("VERMAY_SSH_PORT", "2200")
    monkeypatch.setenv("VERMAY_SSH_IDENTITY_FILE", "~/.ssh/env")
    monkeypatch.setenv("VERMAY_SSH_KNOWN_HOSTS_FILE", "~/.ssh/env_known_hosts")
