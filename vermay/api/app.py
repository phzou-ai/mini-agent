...
from vermay.app_factory import DEFAULT_AGENT_STORE_PATH, DEFAULT_MODEL_CONFIG_PATH, RuntimeFactoryConfig, build_runtime
from vermay.errors import error_info_from_exception, public_error_payload
from vermay.env_config import load_prefixed_env
from vermay.langgraph_runtime import build_graph_model_client
...
def create_app(
    *,
    ...
    app = FastAPI(title="Vermay API", version="0.1.0", lifespan=lifespan)
...
def _router_model_name_override() -> str | None:
    value = load_prefixed_env("VERMAY_ROUTER_").get("VERMAY_ROUTER_MODEL")
    if value is None:
        return None
    value = value.strip()
